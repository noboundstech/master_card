...
.get(function (req, res) {
	...
	async.series([
		...
		function(callback){
			...
			db_query     = require('db_query/query');
			//************* Category ************
			var    sqlstring  = "select distinct categoryDesc from tCategory"+ ' where categoryDesc IS NOT NULL';
			db_query.RunSelSqlFromDb(req,res,sqlstring,response_data,function(){
		...
		function(callback){
			...
			db_query     = require('db_query/query');
			//************* Merchant id /name ************
			var    sqlstring  = "select distinct merchantName ,merchantId from tOfferbyMerchants"+ ' where merchantName IS NOT NULL';
			db_query.RunSelSqlFromDb(req,res,sqlstring,response_data,function(){
	...
	function(callback){
		...
		db_query     = require('db_query/query');
		//************* Merchant id /name ************
		var    sqlstring  = "select distinct offer_rule_en ,offerId from tOfferbyMerchants"+ ' where offer_rule_en IS NOT NULL';
		db_query.RunSelSqlFromDb(req,res,sqlstring,response_data,function(){
			var len_offer=response_data.details.length;
			if(len_offer>0)
			{
				response_data.merchant=[];
				for (t_offer=0;t_offer<len_offer;t_offer++)
				{
					response_data.merchant.push({
						"name" :response_data.details[t_offer].offer_rule_en,
						"id" :response_data.details[t_offer].offerId
					});
				}
				callback();
			}
			...
			{
				response_data.success = false;
				response_data.message = "Offer data row not present in DB table.";
				res.status(203).send({response_data});
			}
		})
	},
	function(callback){
		...
		db_query     = require('db_query/query');
		//************* tag desc ************
		var    sqlstring  = "select distinct tagDesc ,tagId from tTagMaster"+ ' where tagDesc IS NOT NULL';
		db_query.RunSelSqlFromDb(req,res,sqlstring,response_data,function(){
			...
			if(len_tag>0)
			{
				...
				{
					response_data.tag.push({
						"name" :response_data.details[t_tag].tagDesc,
...
db_query.RunSelSqlFromDb(req,res,sqlstring,response_data,function(){
            ...
            if (response_data.details.length > 0)
            {
                ...
                if (age_group_Yflag=='Y' || age_group_Xflag=='Y')
                {
                 ...
                 for (age=0;age<response_data.details.length;age++)
                 {
                     ...
                     if(detail_age >= 65 )
                    {
                                         if (age_group_Yflag == 'Y')
                                          ...
                                          { response_data.details[age].y_value='65 to 100'; }
                                         else
                                          { response_data.details[age].x_value='65 to 100';}
                    }
                    else if(detail_age >= 55 && detail_age<=64)
                    {
                              if (age_group_Yflag == 'Y')

                                          { response_data.details[age].y_value='55 to 64';}
                              else
{response_data.details[age].x_value='55 to 64';}
                    }
                    else if(detail_age >= 45 && detail_age <=54)
                    {
                              if (age_group_Yflag == 'Y')

                                     { response_data.details[age].y_value='45 to 54'; }
                              else
                                     { response_data.details[age].x_value='45 to 54';}
                    }

                    else if(detail_age >= 35 && detail_age <=44)
                    {
                         if (age_group_Yflag == 'Y')

                                     { response_data.details[age].y_value='35 to 44';}
                         else
                                     { response_data.details[age].x_value='35 to 44';

                                 }

                    }
                    else if(detail_age >= 25 && detail_age <=34)
                    {
                           if (age_group_Yflag == 'Y')

                           { response_data.details[age].y_value='25 to 34';}
                            else
                           {
response_data.details[age].x_value='25 to 34';

                           }

                    }
                     else if(detail_age >= 18 && detail_age<=24)
                    {
                    if (age_group_Yflag == 'Y')

                           {response_data.details[age].y_value='18 to 24';}
                                         else
                           {
response_data.details[age].x_value='18 to 24';
             ...
             //  write data to matrix from result set
             ...
             //  write matrix  to send response array
           } // end of if
            else
                {
                    response_data.success = false;
            ...
            for(y_cnt=0; y_cnt < y_axis_array.length;y_cnt++)
             {
                      var name_sent=y_axis_array[y_cnt].name+'_Sent';
                       var name_used=y_axis_array[y_cnt].name+'_Used';
                       var name_clicked=y_axis_array[y_cnt].name+'_Clicked';
