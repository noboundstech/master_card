        ...
        function(callback){
            ...
            db_query.RunSelSqlFromDb(req,res,sqlstring,response_data,function(){
                ...
                if(len_tag>0)
                {
                    ...
                    {
                    response_data.tag.push({       "name" :response_data.details[t_tag].tagDesc,"id" :response_data.details[t_tag].tagId});
                    }
                    callback();
                }
                else
    ...
    async.series([
        function(callback) {
            ...
            validate.validate_fromto_dates(req,res,function(){
                x_field         = req.body.x_axis,
                y_field         = req.body.y_axis,
    ...
    async.series([
        ...
        function(callback) {
            ...
            for(x_cnt=0;x_cnt < x_axis_array.length;x_cnt++)
            {
                ...
                for(y_cnt=0;y_cnt < y_axis_array.length;y_cnt++)
                {
                    ydata.push({yname: y_axis_array[y_cnt].name,sent :0,used :0,click:0});
                }
                xheader.push({ydata});
                graph1.push({xheader});
            }
            ...
            db_query.RunSelSqlFromDb(req,res,sqlstring,response_data,function(){
                ...
                if (response_data.details.length > 0)
                {
                    ...
                    // table is 'M' and 'F'. this need to be set to 'Male' 'Female'
                    //********************************************************
                    if (age_group_Yflag=='Y' || age_group_Xflag=='Y' || gender_Yflag=='Y'    || gender_Xflag=='Y')
                    {
                        for (var age=0;age<response_data.details.length;age++)
                        {
                            //**************************************************
                ...
                response_data.Graph_data =[{ x_axis_name ,y_array_header}];
                if (response_data.details.length > 0)
                {
                    callback();
                }
